...
public class FixedInterpreter extends Interpreter<BasicValue> implements Opcodes {
  ...
  public BasicValue newOperation(final AbstractInsnNode node) throws AnalyzerException {
    return switch (node.getOpcode()) {
      ...
      case LDC -> {
        ...
        if (value instanceof ConstantDynamic dynamic) yield newValue(Type.getType(dynamic.getDescriptor()));
        throw new AnalyzerException(node, "Illegal LDC value " + value);
      }
  ...
  @Override
  public void returnOperation(final AbstractInsnNode node, final BasicValue value, final BasicValue expected) {
  }

  private static boolean isReferenceType(BasicValue value) {
  ...
  public BasicValue merge(final BasicValue value1, final BasicValue value2) {
    ...
    if (isReferenceType(value1) && isReferenceType(value2)) return BasicValue.REFERENCE_VALUE;
